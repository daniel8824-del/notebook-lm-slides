---
name: notebook-lm
description: "NLM 슬라이드 커스텀 제작. A/B 바구니, 5단계 워크플로우, 49종 스타일 키워드, JSON Slide Master, 소스 품질 체크리스트, 실전 사례. Activates on explicit /notebook-lm, 노트북LM, NotebookLM, NLM, or intent like 'NLM으로 슬라이드 만들어줘'"
---
...
| 단계 | 도구 | 아웃풋 |
|------|------|--------|
| ⓪ 노트북 생성 | `notebooklm create "Slides: [주제]"` | NLM 노���북 |
...
python3 ./scripts/extract_nlm_v7.py ./slides.pdf --output ./extracted/
node ./scripts/render_nlm_v7b.js ./extracted/ --output ./output.pptx
python3 ./scripts/qa_nlm_v7.py ./output.pptx --original ./slides.pdf
```
...
> 아래 Step 1~5는 상단 파이프라인의 **⓪ 노트북 생성 ~ ④ 슬라이드 생성** 구간을 상세 설명한다. ⑤~⑧(PDF 추출 → PPTX)은 "NLM 슬라이드 PDF → 편집 가능한 PPTX 추출" 섹션 참조.

**역할 분담:** AI가 80%(취합·분석·요약·생성·디자인), 사람이 20%(인사이트 도출·스토리텔링·최종 검토)

### Step 1: 소스 준비 (A/B 바구니 시스템)

소스를 **두 바구니**로 분류하여 준비한다. (아래 '소스 품질 체크리스트' 참조)
...
**NLM 웹 UI에서 메모 → 소스 전환:** 저장된 메모의 더보기(⋯) → '소스로 변환' 클릭. 소스 이름 앞에 `+`를 붙여 상단 정렬. CLI에서는 `--save-as-note`로 메모 저장 후 웹 UI에서 소스 변환한다.
...
## NLM 슬라이드 PDF → 편집 가능한 PPTX 추출 (v7 파이프라인)

NLM에서 생성한 슬라이드 PDF를 v7 스크립트로 추출·렌더링·QA하여 편집 가능한 PPTX를 만든다. **이 스킬 내에서 완결되며, 다른 스킬 전환 불필요.**
...
# ⑥ Gemini Vision 멀티패스 추출 — 텍스트/이미지/레이아웃을 JSON으로 분리
python3 ./scripts/extract_nlm_v7.py ./slides.pdf --output ./extracted/

# ⑦ Role 기반 템플릿 렌더링 — JSON → 편집 가능한 PPTX
node ./scripts/render_nlm_v7b.js ./extracted/ --output ./output.pptx

# ⑧ QA — 원본 PDF와 비교, 95점+ 목표
python3 ./scripts/qa_nlm_v7.py ./output.pptx --original ./slides.pdf
```
...
**각 스크립트의 역할:**
1. `extract_nlm_v7.py` — PDF → 이미지 변환 → Gemini Vision API가 각 페이지의 텍스트, 이미지, 위치/크기를 멀티패스로 분석 → JSON 출력
2. `render_nlm_v7b.js` — 추출된 JSON을 Role 기반 템플릿 엔진으로 읽어 PptxGenJS로 편집 가능한 PPTX 생성
3. `qa_nlm_v7.py` — 생성된 PPTX와 원본 PDF를 비교하여 점수 산출. 95점 미만이면 재추출/재렌더링
